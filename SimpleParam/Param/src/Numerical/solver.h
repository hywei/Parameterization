...
public:
	...
	void add_coefficient(int index_, double a_) {m_equation.push_back(make_pair(index_, a_));}

	//
	double right_b() {return m_right_b_;}
	vector<pair<int, double> > get_constraint_coef() {return m_equation;}
private:
	vector<pair<int, double> > m_equation;
	double m_right_b_;
