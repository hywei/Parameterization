...
namespace PARAM
{
	...
	//	FormTriPatch();
        ...
        for(size_t k=0; k<m_chart_array.size(); ++k){
            ...
            chart.m_patch_id = k;            
			SetQuadChartConnerParamCoord(k);
//			SetTriangleChartConnerParamCoord(k);
...
//             }                
	...
	void ChartCreator::SetQuadChartConnerParamCoord(int chart_id)
	{
		...
		std::vector<ParamCoord>& conner_pc_vec = m_chart_array[chart_id].m_conner_param_coord_array;
...
//		conner_pc_vec[2] = ParamCoord(0.5, sqrt(3.0)/2); 
	...
	void ChartCreator::SpliteQuadPatchToTriPatch(int patch_id)
	{
		...
		int val1 = max(val[0], val[2]), val2 = max(val[1], val[3]);
		if(val1 > val2){
			int tmp = cur_edges[0];
