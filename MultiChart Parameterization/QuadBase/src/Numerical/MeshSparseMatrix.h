// MeshSparseMatrix.h: interface for the CMeshSparseMatrix_2 class.
//
...
public:
	bool LoadSparseMatrix(string filename);
	bool SaveSparseMatrix(string filename);
...
public:
	...
	void MultiplyDouble4Row(vector<double> weight);       // matrix * vector<double>
	void InsertMatrix(int LURow, int LUCol, CMeshSparseMatrix& inmtx);
 
	//
	void GetMatrixLesseqCol(int col, vector<pair<int, double> >& colInfo);
